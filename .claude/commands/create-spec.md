...
Check both local and remote branches to determine whether
...
Continue until an unused branch name is found.
...
Do not continue or attempt to recover automatically.
...
- `app.py` — existing routes and structure(if exists)
- `database/db.py` — existing schema and functions(if exists)

Inspect `.claude/specs/` and identify relevant existing specs.
Read relevant specs in full to avoid duplicating existing work.

Check `CLAUDE.md` to confirm:
1. The requested step exists.
2. The requested step is not already marked complete.
3. All prerequisite steps are complete.

If the requested step is already complete, warn the user and stop.

If a prerequisite step is incomplete, tell the user which step
is blocking the requested feature and stop.

Do not modify application code, templates, database code,
configuration, dependencies, or tests during this step.
...
objectively verifiable through the application, automated tests,
database inspection, or code inspection.
---

## Step 8 — Save the spec
Save to: `.claude/specs/<step_number>-<feature_slug>.md`

The only intended repository modification is the new spec file.
